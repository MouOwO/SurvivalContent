(function () {
    ...
    function render(value){if(disposed)return;value=value||{};var next=Number(value.active)===1?String(value.token)+":"+JSON.stringify(rows(value.cards)):"";current=value;
        ...
        if(reduced)finish();else later(1.60+Math.max(0,cards.length-1)*.11,finish);
    }
    ...
    config.SurvivalRogueReward={Dispose:function(){disposed=true;cancel();life.Dispose();if(CustomNetTables.UnsubscribeNetTableListener)CustomNetTables.UnsubscribeNetTableListener(tableSubscription);if(config.SurvivalUILayers)config.SurvivalUILayers.Close("rogue_choice");},SetReducedMotion:function(enabled){panel("RogueReducedMotion").checked=!!enabled;reduced=!!enabled;config.RogueReducedMotion=reduced;if(active()&&!ready&&reduced){cancel();finish();watchFit();}}};
    ...
    panel("RogueRewardReroll").visible=false;panel("RogueRewardReroll").hittest=false; U.Checkbox.Adopt(panel("RogueReducedMotion"));
